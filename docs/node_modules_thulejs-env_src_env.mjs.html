...
<head>
    ...
    <meta charset="utf-8">
    <title>node_modules/thulejs-env/src/env.mjs - ThuleJS</title>
...
<div id="main">
    ...
    <h1 class="page-title">node_modules/thulejs-env/src/env.mjs</h1>
...
function logger(level, id, msg, args){
    ...
    if(level &lt;= 40) return errorMessage;
}
/**
...
export class _env {
    ...
    static getEnv(envKey){
        if(!envKey){ return _env.#env};
        if (_env.#env[envKey]) return _env.#env[envKey]
        return logger(30, 'getEnv', 'key not found: ' + envKey )
    }
    ...
    static setEnv(key, value) {
        if (Object.isFrozen(_env.#env)) return logger(30, 'setEnv', 'checkin active')
        if (Object.keys(_env.#env).includes(key)) return logger(30, 'setEnv', 'key already exists: ' + key + ': ' + _env.#env[key])
        _env.#env[key] = value
        return true
    }
    ...
    static updateEnv(key, value) {
        if (Object.isFrozen(_env.#env)) return logger(30, 'setEnv', 'checkin active')
        if (!Object.keys(_env.#env).includes(key)) return logger(30, 'updateEnv', 'key not found: ' + key )
        _env.#env[key] = value
        return true
    }
    ...
    static delEnv(key) {
        if (Object.isFrozen(_env.#env)) return logger(30, 'setEnv', 'checkin active')
        if (!Object.keys(_env.#env).includes(key)) return logger(30, 'delEnv', 'key not found: ' + key )
        delete _env.#env[key]
        return true
    }
